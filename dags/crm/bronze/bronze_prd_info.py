...
with DAG(
    dag_id="crm_bronze_prd_info",
    schedule=None,
) as dag:

    @task
    def extract_prd_info():
        ...
        input_path = Path("/opt/airflow/data/source/crm/prd_info.csv")

        staging_path = (
            Path("/opt/airflow/data/bronze/crm/prd_info")
            / partition_date
            / "prd_info.parquet"
        )
    ...
    @task
    def load_prd_info(extract_result):
        ...
        object_name = f"bronze/crm/prd_info/{partition_date}/prd_info.parquet"
    ...
    staging_path = extract_prd_info()

    load_prd_info(staging_path)
